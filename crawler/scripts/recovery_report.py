...
def amount_metrics(projects):
    ...
    for kind in ('预算','限价','成交','合同'):
        ...
        totals[kind]={'yuan':total,'records':count}
    return totals
...
def build(before_path,batch):
    ...
    for f in (ROOT/'data/public-downloads').glob(batch+'*/manifest.json'):
        a=json.loads(f.read_text());fresh=[r for r in a if not r.get('reused_saved_evidence')]
        batches.append({'batch':f.parent.name,'attempted':len(fresh),'responses':sum(r['status']=='ok' for r in fresh),'reused':len(a)-len(fresh)})
    for f in (ROOT/'data/cloud-downloads').glob(batch+'*/manifest.json'):
        ...
        batches.append({'batch':f.parent.name+'（云端）','attempted':len(a),'responses':sum(r['status']=='ok' for r in a),'reused':0})
    bm,am,wm=metrics(old),metrics(annual),metrics(week)
    week_amounts=amount_metrics(week);previous_amounts=amount_metrics(previous)
    ...
    result['previousWeekAmounts']=previous_amounts
    ...
    batch_table=''.join('<tr><td>'+h(x['batch'])+'</td><td>'+str(x['attempted'])+'</td><td>'+str(x['responses'])+'</td><td>'+str(x['reused'])+'</td></tr>' for x in batches)
    ...
    page=page.replace('<th>已尝试 URL</th><th>取得响应</th>','<th>本轮请求 URL</th><th>本轮取得响应</th><th>复用已存响应</th>')
    page=page.replace('<h2>仍需补充的正文</h2>','<h2>仍需补充的正文</h2><p>'+h(progress_text)+'</p>')
    previous_money='<table><tr><th>金额口径</th><th>可汇总样本金额</th><th>记录数</th></tr>'+''.join('<tr><td>'+h(k)+'</td><td>'+format(v['yuan']/10000,',.2f')+' 万元</td><td>'+str(v['records'])+'</td></tr>' for k,v in previous_amounts.items())+'</table><p>仅汇总正文已取得且口径明确的样本；各类金额不可相加，不代表完整周市场总额。</p>'
    previous_section='<h2>最近一个完整周</h2><p>'+h(previous_range['start'])+' — '+h(previous_range['end'])+'，共 '+str(previous_metrics['records'])+' 条记录，其中正文已取得 '+str(previous_metrics['body_obtained'])+' 条，教育类 '+str(previous_metrics['education'])+' 条，已取得正文涉及 '+str(previous_metrics['buyers'])+' 家采购单位。</p>'+previous_money+table(previous)
...
.report-nav-inner{max-width:1250px;margin:auto;display:flex;align-items:center;gap:28px;padding:0 24px;min-height:70px}
.report-brand{font-weight:650;text-decoration:none;margin-right:auto;font-size:17px;white-space:nowrap}
.report-links{display:flex;align-items:stretch;gap:24px;align-self:stretch}
...
@media(max-width:900px){.report-nav-inner{flex-wrap:wrap;gap:0;padding:14px 20px 0}.report-brand{font-size:17px;padding-bottom:12px}.report-links{width:100%;justify-content:space-between;gap:12px}.report-links a{padding:10px 0;font-size:14px}}
@media(max-width:600px){.report-nav-inner{padding:14px 14px 8px}.report-links{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:4px 8px}.report-links a{justify-content:center;padding:8px 0;border-bottom:0}.report-links a[aria-current=page]{background:#e7eee5}.annual-intro h1{font-size:28px}.annual-summary{grid-template-columns:repeat(2,1fr);row-gap:18px;padding:18px 0}.annual-summary div{padding:0 16px}.annual-summary div:nth-child(3){padding-left:0}.annual-summary div:nth-child(2){border:0}.annual-summary strong{font-size:27px}.annual-tools select{flex:1 1 130px;min-width:0;max-width:none}.annual-tools button{flex:0 0 auto}.annual-caption{flex-direction:column;gap:4px}}
"""
    def navigation(active):
        links=[('overview','index.html#overview','首页'),('report','recovery-report.html','市场周报'),('annual','annual-projects.html','近一年明细'),('projects','index.html#projects','项目库'),('coverage','index.html#coverage','覆盖情况'),('method','index.html#method','实施方案')]
        return '<header class="report-nav"><nav class="report-nav-inner" aria-label="主导航"><a class="report-brand" href="index.html#overview" aria-label="心育采购观察首页">心育采购观察</a><div class="report-links">'+''.join('<a href="'+url+'"'+(' aria-current="page"' if key==active else '')+'>'+label+'</a>' for key,url,label in links)+'</div></nav></header>'
